...
- 07-16 QA 루프(워크플로우: 도구별 QA→수정→재QA 최대 3라운드) + 가이드 재생성:
  QA에서 compress 버그 1건 발견·수정 — 목표 용량 모드의 투명 PNG가 잠긴 프리셋 해상도에
  묶여 결과가 달라지고 목표 미달로 실패하던 문제 → 프리셋 독립 2차원 사다리(해상도×색 수)로 교체.
  merge·hangul은 1라운드 무결함. 가이드 3종 스크린샷 재촬영(새 UI: 32px 호버 툴바·변환 중
  스켈레톤·프리셋 잠금·시스템 파일 배너) + PDF 3종 재생성(각 4쪽, pdf 렌더 검증) + 링크 검사 통과.
